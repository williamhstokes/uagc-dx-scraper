...
This repository is structured so GitHub Pages deploys the `public/` folder to:

```text
https://williamhstokes.github.io/uagc-webscraper/
```

The static dashboard, CSS, JavaScript, and image assets will load from GitHub Pages. The crawler API still requires the Node server in `server.js`, so scan actions such as `/api/scan/start` will not run on GitHub Pages. Use `npm start` locally or deploy the Node app to a Node-capable host for full crawler functionality.
